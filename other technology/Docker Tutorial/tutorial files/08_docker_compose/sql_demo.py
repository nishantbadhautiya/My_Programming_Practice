...
def create_connection():
    return pymysql.connect(
        # host="localhost" # used to connect with localhost mysql server (in this case localhost mysql server is my own computer) 
        # host="172.17.0.2", # connect to another docker container with ip_address (172.17.0.2) we can get it by running (docker logs mysqldb) 
        host="mysqldb", # use it with docker network and mysqldb is the name of running container of mysql 
